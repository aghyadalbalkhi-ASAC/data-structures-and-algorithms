...
const templatingWithMustache = () => {
  ...
  characters.forEach(value => {
    let sectionItem = Mustache.render(template, value);
    arr.push(sectionItem);
...
const hasChildrenEntries = (arr, character) => {
  ...
  arr.forEach(value => {
    ...
    let arrofValue = Object.entries(value);
...
const houseSize = (arr) => {
  ...
  // Solution code here...
  arr.forEach(v =>{
    let newVa = new HouseSize(v);
    newVa.memberCalc();
  });

  function HouseSize(objectHouse){
    this.house = objectHouse.house;
    this.members=memberCalc();
    sizes.push(this);
  }

  HouseSize.prototype.memberCalc = function() {
    let count =0;
    ...
    return count -2 + this.children.length;
  };
